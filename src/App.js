...
function Food({name, picture}) { // props를 인자로 받으면, Object 안에 favorite이라는 변수가 있음. 왼쪽 같이 표현하는 것은 ES6 문법임.
  return (
    <div>
      <h1>I love {name}</h1>
      <img src={picture} />
...
function App() {
  return (
    <div>
      {foodILike.map((food, idx) => (
        <Food name={food.name} picture={food.image} key={idx} />
      ))}
    </div>
  );
}
